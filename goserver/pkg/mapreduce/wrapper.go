...
func MapReduceWithFilePaths(filePaths []string) cmap.ConcurrentMap[string, *roaring.Bitmap] {
	...
	_, _ = MapReduce(func(source chan<- []byte) {
		...
		fmt.Println("✅ Reducer worker 启动, 聚合数据为cmap的bitmap")
...
// 存储发生改变，存在cmap[string][]int, 后续存redis
func MapReduceWithFilePathsToNormalCmap(filePaths []string) cmap.ConcurrentMap[string, []int] {
	invertedIndex := cmap.New[[]int]()
	...
	_, _ = MapReduce(func(source chan<- []byte) {
		...
		fmt.Println("✅ Reducer worker 启动, 聚合数据为cmap的[]int")
		for values := range pipe {
			//fmt.Println("reduce")
			for _, v := range values {
				if value, ok := invertedIndex.Get(v.Key); ok {
					value = append(value, cast.ToInt(v.Value))
					invertedIndex.Set(v.Key, value)
				} else {
					docIds := make([]int, 0)
					docIds = append(docIds, cast.ToInt(v.Value))
					invertedIndex.Set(v.Key, docIds)
				}
			}
		}
